...
// Same inflow as get_states, but with a yaw attitude and non-zero angular velocities
BodyStates get_states_at_heading(const double psi, const double u, const double v)
{
    ...
    states.w.record(0, 0);
    states.p.record(0, 0);
    states.q.record(0, 0.1);
    states.r.record(0, 0.05);
    states.qr.record(0, std::cos(psi/2));
    states.qi.record(0, 0);
    states.qj.record(0, 0);
...
TEST_F(HydroPolarForceModelTest, force_must_not_depend_on_vessel_heading)
{
    ...
    env.rot = YamlRotation("angle", {"z","y'","x''"});
    const HydroPolarForceModel force_model(input, "body", env);
    // A foil only sees the flow in its own frame, so a fixed body-frame inflow must give the same
    // force whatever the vessel's compass heading
    const auto F0  = force_model.get_force(get_states_at_heading(0.,        5, 1), 0, env, {});
